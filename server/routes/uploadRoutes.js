...
router.post("/SignPDFWB", async (req, res) => {
  console.log("THE BODY", req.body);
  res.json({ awesome: "It Works" });
  let fileName = req.body.doc.split("/");
  fileName = `public/${fileName[fileName.length - 1]}`;
  ...
  fs.writeFileSync("public/aaaWayneTest.pdf", await pdfDoc.save());
